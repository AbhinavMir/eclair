...
class FunctionAggregator:
    ...
    def read_config(self, config_file_path):
        try:
            ...
            print(f"Config file {config_file_path} not found. Do you want to create a config file? (y/n)")
            answer = input()
            if answer == "y":
                entry = ProjectInitializer()
                entry.initialize_project()
                return self.read_config(config_file_path)
            else:
                print("OK, exiting.")
            sys.exit(1)
